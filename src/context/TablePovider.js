...
function TableProvider({ children }) {
  ...
  useEffect(() => {
    const fetchDataFromApi = async () => {
      const request = await fetch(
        `https://swapi-trybe.herokuapp.com/api/planets/?format=json${query}`,
      );
  ...
  const removeFilter = (elementIndex) => {
    ...
    // const filterByUnknown = data.filter((elem) => elem[column] === 'unknown');
  ...
  const removeAllFilters = () => {
    ...
    setFilterColumns([
      ...
      'rotation_period',
      'surface_water']);
  };// Refatorar com um estado inicial
